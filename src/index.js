// Cloudflare Worker Entry Point - MongoDB Atlas - Stateless V2.0.0
...
async function handleApiRequest(request, env) {
    ...
    if (!connectionString) {
        return jsonResponse({
            error: true,
            message: 'Failed to connect to MongoDB: MONGODB_URI environment variable is missing.'
        }, 500);
    }

    let client = null;
    try {
        // Connect to MongoDB Atlas (Stateless - connection closes at end of request)
        client = new MongoClient(connectionString, {
            serverSelectionTimeoutMS: 5000,
            socketTimeoutMS: 30000
        });
        await client.connect();
        const database = client.db('hamdhoon'); // Database name
        ...
        // Only run seeder on app initialization or login to avoid overhead on CRUD
        if (action === 'init_app' || action === 'login') {
            try {
            ...
            case 'save_user_dir': {
                ...
                for (let u of users) {
                    ...
                    if (existing) {
                        ...
                        const passHash = u.passwordHash || '240be518fabd2724ddb6f04eeb1da5967448d7e831c08c8fa822809f74c720a9';
        ...
        return jsonResponse({
            error: true,
            message: 'Database query failed: ' + apiError.message
        }, 500);
    } finally {
        if (client) {
            try {
                ...
                console.error('Failed to close MongoClient:', closeError);
            }
        }
    }
}
